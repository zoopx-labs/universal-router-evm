...
contract RouterSharesTest is Test {
    ...
    bytes32 messageHash = Hashing.messageHash(1, address(adapter), address(0), address(token), amount, keccak256(abi.encodePacked("p")), 42, 2);
    ...
    // New model: destination finalize emits zero fee distribution; entire amount forwarded to vault (adapter here)
    vm.prank(address(adapter));
    router.finalizeMessage(gri, messageHash, address(token), address(adapter), lp, amount, 0, 0);

    // No on-destination fee movement
    assertEq(token.balanceOf(FEE) - beforeCollector, 0);
    assertEq(token.balanceOf(lp) - beforeLP, 0);
    assertEq(token.balanceOf(address(adapter)) - beforeAdapter, amount); // full amount
